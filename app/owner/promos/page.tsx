...
type OutletOption = { id: string; name: string; is_coming_soon?: boolean | null };
...
export default function OwnerPromoBannersPage() {
  ...
  const [outletIds, setOutletIds] = useState<string[]>([ALL_OUTLET_TARGET]);
  const [isActive, setIsActive] = useState(true);

  const activeOutlets = outlets.filter((o) => !isComingSoonOutlet(o));
  const allSelected =
    outletIds.includes(ALL_OUTLET_TARGET) ||
    (activeOutlets.length > 0 && activeOutlets.every((o) => outletIds.includes(o.id)));
  ...
  const resetForm = () => {
    ...
    setBannerUrl('');
    setPromoCode('');
    setOutletIds([ALL_OUTLET_TARGET]);
    setIsActive(true);
  };

  const load = async () => {
    const [{ data: outletData }, { data: promoData, error }] = await Promise.all([
      supabase.from('outlets').select('id, name, is_coming_soon').order('name'),
  ...
  const fillEdit = (row: PromoRow) => {
    ...
    setOutletIds(normalizeOutletIds(parseTargetOutletIds(row.target_outlet_ids, row.outlet_id)));
    setIsActive(row.is_active !== false);
  };

  const toggleAllOutlets = () => {
    setOutletIds(allSelected ? [] : [ALL_OUTLET_TARGET]);
  };

  const toggleOutlet = (id: string) => {
    const current = allSelected ? activeOutlets.map((o) => o.id) : outletIds.filter((x) => x !== ALL_OUTLET_TARGET);
    const next = current.includes(id) ? current.filter((x) => x !== id) : [...current, id];
    if (activeOutlets.length > 0 && activeOutlets.every((o) => next.includes(o.id))) {
      setOutletIds([ALL_OUTLET_TARGET]);
      return;
    }
    setOutletIds(next);
  ...
  const outletLabelsOf = (row: PromoRow) => {
    const ids = parseTargetOutletIds(row.target_outlet_ids, row.outlet_id);
    if (isAllOutletTarget(ids)) return 'Semua outlet';
    const names = ids.map((id) => outlets.find((o) => o.id === id)?.name || 'Outlet').filter(Boolean);
    return names.length ? names.join(', ') : 'Outlet';
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const selectedIds = normalizeOutletIds(outletIds);
    const specificIds = selectedIds.filter((id) => id !== ALL_OUTLET_TARGET);
    const payload = {
      title: title.trim(),
      description: description.trim(),
      banner_url: bannerUrl.trim() || null,
      promo_code: code,
      target_outlet_ids: selectedIds,
      outlet_id: specificIds[0] || null,
      is_active: isActive
    };
    const withoutCode = {
      title: payload.title,
      ...
      target_outlet_ids: payload.target_outlet_ids,
    ...
    if (editId === 'NEW') {
      const res = await insertWithFallback('promotions', [
        payload,
        withoutCode,
        { title: payload.title, description: payload.description, banner_url: payload.banner_url, target_outlet_ids: payload.target_outlet_ids, outlet_id: payload.outlet_id, is_active: payload.is_active },
        { title: payload.title, description: payload.description, banner_url: payload.banner_url, outlet_id: payload.outlet_id, is_active: payload.is_active },
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-5xl">
        <form onSubmit={handleSave} className="bg-white border border-slate-200 rounded-2xl p-4 md:p-6 shadow-sm space-y-3">
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-600 block mb-1">Kode promo</label>
            <input
              value={promoCode}
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-xl bg-slate-50 overflow-hidden">
              <label className="flex items-center gap-2 px-3 py-2.5 text-xs font-black text-slate-800 border-b border-slate-200">
                <input type="checkbox" checked={allSelected} onChange={toggleAllOutlets} />
                Semua Outlet
              </label>
              <div className="max-h-48 overflow-y-auto divide-y divide-slate-100">
                {activeOutlets.length === 0 && (
                  <p className="px-3 py-2 text-[11px] text-slate-400">Belum ada outlet aktif.</p>
                )}
                {activeOutlets.map((o) => {
                  const checked = allSelected || outletIds.includes(o.id);
                  return (
                    <label key={o.id} className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-slate-700">
                      <input type="checkbox" checked={checked} onChange={() => toggleOutlet(o.id)} />
                      {o.name}
                    </label>
                  );
                })}
            ...
            <p className="text-[10px] text-slate-400 mt-1">Banner tampil di Home pelanggan jika target berisi ALL atau outlet yang sedang dipilih pelanggan.</p>
          </div>
